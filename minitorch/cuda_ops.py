...
def tensor_zip(fn: Callable[[float, float], float]) -> Callable:
    """CUDA higher-order tensor zip function with improved broadcasting support."""
        ...
        def _kernel(out, out_shape, out_strides,
            ...
            if idx < len(out):
                ...
                for i in range(len(out_shape)):
                    a_index[i] = out_index[i] if i < len(a_shape) and a_shape[i] > 1 else 0
                    b_index[i] = out_index[i] if i < len(b_shape) and b_shape[i] > 1 else 0
...
def tensor_reduce(fn: Callable[[float, float], float]) -> Callable:
    """CUDA tensor reduction function with improved broadcasting support."""
    def _reduce(out: Storage, out_shape: Shape, out_strides: Strides,
        ...
        def _kernel(out, out_shape, out_strides,
            ...
            shared = cuda.shared.array(shape=THREADS_PER_BLOCK, dtype=numba.float64)
            ...
            if idx < size:
                ...
                # Reduction loop with broadcasting
                for j in range(a_shape[reduce_dim]):
                    # Create input index considering broadcasting
                    in_index = cuda.local.array(MAX_DIMS, numba.int32)
                    for i in range(len(out_shape)):
                        if i == reduce_dim:
                            in_index[i] = j
                        else:
                            in_index[i] = out_index[i] if i < len(a_shape) and a_shape[i] > 1 else 0
